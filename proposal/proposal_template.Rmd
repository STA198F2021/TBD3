...
reasons <-c("Work", "Provide Care for Others", "Grocery Shopping", "Essential Shopping", "Exercise", "Walking Dog", "Other")
...
tidy_data <- tidy_data %>%
  ...
  mutate(americanindian = ifelse(race == "American Indian", 1, 0)) %>%
  mutate(mixed = ifelse(race == "Mixed", 1, 0)) %>%
  mutate(hawaiian = ifelse(race == "Native Hawaiian", 1, 0)) %>%
  ...
  mutate(at_least_college = ifelse(educ == 6, 1, 0))
...
t.test(tidy_data$localsiphours~tidy_data$at_least_college, var.equal=FALSE)
...
localsiphours_fit_education <- linear_reg() %>%
  set_engine("lm") %>%
  fit(localsiphours ~ g9_11 + technical_college + at_least_college, data = tidy_data)
